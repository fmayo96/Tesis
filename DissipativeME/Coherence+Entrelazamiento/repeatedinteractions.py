...
class System():
    ...
    def Dissipator(self, bath, V):
            ...
            return -0.5 * partial_trace(Conmutator, self.dim, self.dim)
    ...
    def Time_evolution(self, bath, bath_hamiltonian, tf, dt, V):
        ...
        for i in tqdm(range(self.prev_steps, self.prev_steps + N - 1)):
            ...
            Conmutator = 0.5 * (np.dot(V,np.dot(V,np.kron(np.eye(self.dim),bath_hamiltonian)))-np.dot(V,np.dot(np.kron(np.eye(self.dim),bath_hamiltonian),V))-np.dot(V,np.dot(np.kron(np.eye(self.dim),bath_hamiltonian),V))+np.dot(np.kron(np.eye(self.dim),bath_hamiltonian),np.dot(V,V)))
