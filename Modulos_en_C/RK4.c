...
void RK4_open(double complex *propagator, double complex *dissipator, double complex *state, double complex *hamiltonian, double complex *bath_state, double complex *bath_hamiltinian, double complex *interaction, double dt, int dim, int step)
{
    ...
    for(i = 0; i < dim*dim; i++)
    {
        ...
        *(step_hamiltonian + i) = *(hamiltonian + step * dim * dim + i);
    }
    Dissipator(dissipator, step_state, bath_state, interaction, dim);
    Propagator(propagator, step_state, step_hamiltonian, dissipator, dim);
    for(i = 0; i < dim*dim; i++)
    {
    ...
    Propagator(propagator, aux_state, step_hamiltonian, dissipator, dim);
    for(i = 0; i < dim*dim; i++)
    {
...
void RK4_closed(double complex *propagator, double complex *state, double complex *hamiltonian, double dt, int dim, int step)
{
    ...
    Unitary(propagator, step_state, step_hamiltonian, dim);
    for(i = 0; i < dim*dim; i++)
    {
        *(K1 + i) = *(propagator + i) * dt;
        *(aux_state + i) = *(step_state + i) + 0.5 * *(K1 + i);
